...
// Plain ANSI replacement for espressif/esp-dsp's SIMD dot product.
// Model is tiny (260K params) so this is plenty fast without the extra
// component dependency.
static inline void dsps_dotprod_f32_aes3(const float *src1, const float *src2, float *dest, int len)
{
    float sum = 0.0f;
    for (int i = 0; i < len; i++)
    {
...
// Output capture buffer so generate() results can be returned over HTTP
// instead of only printed to the serial console.
char *g_llm_output_buf = NULL;
size_t g_llm_output_len = 0;
size_t g_llm_output_cap = 0;
...
void generate(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler, char *prompt, int steps, generated_complete_cb cb_done)
{
    ...
    while (pos < steps)
    {
        ...
        fflush(stdout);
        if (g_llm_output_buf != NULL)
        {
            size_t plen = strlen(piece);
            if (g_llm_output_len + plen < g_llm_output_cap)
            {
                memcpy(g_llm_output_buf + g_llm_output_len, piece, plen);
                g_llm_output_len += plen;
                g_llm_output_buf[g_llm_output_len] = '\0';
            }
        }
        token = next;

        // Yield briefly each token so the idle task on this core still gets
        // scheduled and the Task Watchdog Timer never starves.
        vTaskDelay(1);
